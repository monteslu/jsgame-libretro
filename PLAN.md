...
- The privileged JS runtime is **embedded in the binary**: CMake zips `runtime/`
  → `runtime.zip`, `scripts/bin2c.js` codegens it into a `const unsigned char[]`
  (`embedded_runtime.c`, portable across gcc/clang/MSVC — no objcopy/incbin). On
  first content load the core extracts the zip to a temp dir once
  (`src/runtime_unzip.c`, vendored miniz) and feeds that dir to the existing
  `runtime_dir` loader — so CJS `require`, dynamic ESM `import`, `import.meta`,
  the `.wasm` asset, and `new Worker(path)` all resolve from a real directory
  with zero JS changes. The temp dir is version-stamped by a crc32 of the zip
  (`jsgame-runtime-<tag>`) and reused if already populated. Dev builds set
  `JSGAME_RUNTIME_DIR` to load `runtime/` from disk for fast iteration (override).
...
│   ├── embedded_runtime.h       ← extern blob symbols + extract fn decl
│   ├── runtime_unzip.c          ← extract embedded zip → temp dir (miniz)
│   ├── vendor/miniz.{c,h}       ← single-file zip reader (zlib-APIs off)
│   └── (build/embedded_runtime.c ← generated byte array, gitignored)
...
JS dev loop: the build always embeds the runtime blob (zip+codegen is a few
seconds), but setting `JSGAME_RUNTIME_DIR=$PWD/runtime` makes the core load
`runtime/` straight from the source tree (overrides the embedded blob, §4.3), so
shim development is edit-and-relaunch with no C rebuild.
