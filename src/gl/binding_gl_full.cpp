// binding_gl_full.cpp — full GLES 3.0 / WebGL2 binding, ported from
// native-gles (gl_bindings.cpp) but registered as the jsgame_gl linked
// binding. GL calls go straight to the libGLESv2 the core links; the frontend
// owns the actual context (RETRO_HW_CONTEXT_OPENGLES3). webgl-node's
// webgl2-context.mjs sits on top of these flat glXxx functions in the realm.
#include <napi.h>
#include "gl_bindings.h"

// Context lifecycle is the frontend's job in a libretro core; these are stubs
// so engine code that probes them no-ops cleanly.
static Napi::Value jsg_noop(const Napi::CallbackInfo& info) { return info.Env().Undefined(); }

Napi::Object RegisterFullGL(Napi::Env env, Napi::Object exports) {
  exports.Set("swapBuffers", Napi::Function::New<jsg_noop>(env));
  exports.Set("setSwapInterval", Napi::Function::New<jsg_noop>(env));
  exports.Set("makeCurrent", Napi::Function::New<jsg_noop>(env));
